...
不确定先录什么？从以下内容选择一个小目标，按步骤完成第一段演示：

- [三套制作案例](docs/RECIPES.md)：30 秒核心功能、20 秒版本更新、15 秒交互作品，附建议时间与画面参数。
- [可复制录制脚本](docs/STORYBOARD.md)：先确定观众、关键操作和结果，再开始录屏。
- [发布前质量清单](docs/PUBLISH_CHECKLIST.md)：检查可读性、字幕时机、声音和敏感信息。
- [贡献与反馈指南](CONTRIBUTING.md)：报告可复现问题，或提出有具体场景的功能建议。

这些内容是制作方法和手动配置建议，不是新增的自动模板、多轨功能或效果承诺。
